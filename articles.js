...
const displayTitle = article => {
  ...
  return article.slug === 'Lio-Rush-and-He-who-pushes-the-Blackheart' ? title.replace(/\bHe\b/, '<em>He</em>') : title;
};
const articleBody = body => escapeHtml(body).split('\n').filter(Boolean).map(paragraph => {
  const text = paragraph.trim();
  const image = text.match(/^!\[([^\]]*)\]\((\/assets\/uploads\/[^)\s]+)\)$/);
  return image ? `<figure class="article-image"><img src="${image[2]}" alt="${image[1]}" /></figure>` : `<p>${text}</p>`;
}).join('');
const articleCard = article => `<article class="public-article"><p class="article-meta"><span>${escapeHtml(article.category)}</span><span>${formatDate(article.date)}</span></p><h2><a href="article.html?article=${encodeURIComponent(article.slug)}">${displayTitle(article)}</a></h2><p>${escapeHtml(article.excerpt)}</p><a href="article.html?article=${encodeURIComponent(article.slug)}" class="read-link">Read article <b>→</b></a></article>`;
...
fetch('data/articles.json')
  ...
  .then(data => {
    ...
    if (page) {
      ...
      page.innerHTML = article ? `<article><p class="article-meta"><span>${escapeHtml(article.category)}</span><span>${formatDate(article.date)}</span></p><h1>${displayTitle(article)}</h1><p class="article-excerpt">${escapeHtml(article.excerpt)}</p><div class="article-body">${articleBody(article.body)}</div></article>` : '<p>That article could not be found.</p>';
